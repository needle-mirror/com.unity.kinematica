...
namespace Unity.Kinematica.Editor
{
    internal partial class Builder
    {
        ...
        ITag GetTagInterface(Payload payload)
        {
            var method = GetType().GetMethod(nameof(GetTagInterfaceGeneric),
                BindingFlags.NonPublic | BindingFlags.Static);
            Assert.IsTrue(method != null);
            var genericMethod = method.MakeGenericMethod(payload.Type);
            return (ITag)genericMethod.Invoke(null, new object[] { payload });
        }

        static ITag GetTagInterfaceGeneric<T>(Payload payload) where T : struct
        {
            return payload.GetValue<T>() as ITag;
        }

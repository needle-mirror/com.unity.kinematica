    ...
    internal partial class TaggedAnimationClip : IDisposable
    {
        ...
        public AnimationClip AnimationClip
        {
            get
            {
                if (m_Clip == null)
                {
                    m_Clip = m_AnimationClipReference.asset;
                    if (m_Clip != null)
                    {
            ...
            set
            {
                try
                {
                    m_AnimationClipReference.asset = value;
                }
                catch (ArgumentException)
                {
                    throw new InvalidOperationException("Only AnimationClips that are Assets on disk can be used");
                }
                finally
                {
                    m_Clip = value;
        ...
        public bool Valid
        {
            get { return !m_AnimationClipReference.isBroken && AnimationClip != null; }
        }
        ...
        {
            m_Asset = asset;
            m_AnimationClipReference = new LazyLoadReference<AnimationClip>();
        ...
        public (int, TagAnnotation) GetMetricAndTagCoveringTime(float timeInSeconds)
        {
            ...
            {
                int index = GetMetricIndexCoveringTimeOnTag(TagAnnotation.GetTagTypeFullName(tag));
                if (index >= 0)
                {
                    return (index, tag);
                }

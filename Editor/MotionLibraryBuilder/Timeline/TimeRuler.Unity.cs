...
namespace Unity.Kinematica.Editor
{
    partial class TimeRuler
    {
        ...
        internal const float kTickRulerHeightMax = 0.65f; // height of the ruler tick marks when they are highest
            ...
            TimelineViewMode timeFormat)
        {
            ...
            for (int i = 0; i < m_TickCache.Count; i++)
            {
                ...
                string label;
                if (timeFormat == TimelineViewMode.frames)
                {
                    label = Mathf.RoundToInt(frame).ToString(CultureInfo.InvariantCulture);
                }
                else
                {
                    label = TimelineUtility.TimeToTimeFrameStr(m_TickCache[i], (int)frameRate);
                }

                var labelSize = GUI.skin.GetStyle("label").CalcSize(new GUIContent(label));
                float x = labelPos - labelSize.x * 0.5f + 3;
                float y = 0;

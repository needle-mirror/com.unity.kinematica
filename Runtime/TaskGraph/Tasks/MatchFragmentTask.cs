...
{
    /// <summary>
    /// The match fragment task accepts a list of animation pose sequences
    ...
    /// </summary>
    [Data("MatchFragment", "#2A3756"), BurstCompile]
    public struct MatchFragmentTask : Task, GenericTask<MatchFragmentTask>
    {
        public Identifier<MatchFragmentTask> self { get; set; }
        ...
        internal struct MatchResult
        {
            ...
            public DeviationTable deviationTable;

            public static MatchResult CreateInvalid()
            {
                return new MatchResult()
                {
                    result = Result.Failure,
                    deviationTable = DeviationTable.CreateInvalid()
                };
            }

            public static implicit operator Result(MatchResult result) => result.result;

            public static implicit operator DeviationTable(MatchResult result) => result.deviationTable;
        }
        ...
        internal unsafe MatchResult SelectMatchingPose(bool bComputeDeviationTable = false)
        {
            MatchResult result = MatchResult.CreateInvalid();
            ...
            if (bComputeDeviationTable)
            {
                result.deviationTable = DeviationTable.Create(sequences);
            }
            ...
            for (int i = 0; i < numSequences; ++i)
            {
                ...
                for (int j = 0; j < numFrames; ++j)
                {
                    ...
                    if (bComputeDeviationTable)
                    {
                        result.deviationTable.SetDeviation(i, j, candidateDeviation, -1.0f);
                    }
            ...
            if (!bComputeDeviationTable)
            {
                synthesizer.GetRef<TimeIndex>(this.closestMatch).Ref = closestMatch;
            }

            result.result = closestMatch.IsValid ? Result.Success : Result.Failure;
            return result;
        }
        ...
        internal unsafe MatchResult SelectMatchingPoseAndTrajectory(bool bComputeDeviationTable = false)
        {
            MatchResult result = MatchResult.CreateInvalid();
            ...
            for (int i = 0; i < numSequences; ++i)
            {
                ...
                for (int j = 0; j < numFrames; ++j)
                {
                    ...
                    if (bComputeDeviationTable)
                    {
                        result.deviationTable.SetDeviation(i, j, poseDeviation, trajectoryDeviation);
                    }
        ...
        internal unsafe void ComputeCandidateCost(TimeIndex candidateTimeIndex, out float poseDeviation, out float trajectoryDeviation)
        {
            poseDeviation = -1.0f;
            trajectoryDeviation = -1.0f;
            ...
            Binary.CodeBookIndex codeBookIndex = binary.GetCodeBookAt(candidateTimeIndex);
            if (!codeBookIndex.IsValid)
            {
                return;
            }
            ref Binary.CodeBook codeBook = ref binary.GetCodeBook(codeBookIndex);

            TimeIndex samplingTimeIndex = synthesizer.GetRef<SamplingTime>(this.samplingTime).Ref.timeIndex;

            bool bTrajectoryMatching = false;
            MemoryArray<AffineTransform> trajectory = MemoryArray<AffineTransform>.Null;
            if (this.trajectory.IsValid)
            {
                bTrajectoryMatching = true;
                trajectory = synthesizer.GetArray<AffineTransform>(this.trajectory);
            }

            Query query = Query.Create(ref binary);

            float poseWeight = 1.0f - trajectoryWeight;
            query.DecodePose(ref binary, codeBookIndex, samplingTimeIndex);
            query.GeneratePoseDistanceTable(ref binary, codeBookIndex, poseWeight);
            if (bTrajectoryMatching)
            {
                query.DecodeTrajectory(ref binary, codeBookIndex, trajectory);
                query.GenerateTrajectoryDistanceTable(ref binary, codeBookIndex, trajectoryWeight);
            }

            int fragmentIndex = binary.GetCodeBookFragmentIndex(ref codeBook, candidateTimeIndex);
            Assert.IsTrue(fragmentIndex != -1);

            float* poseDistances = query.GetPoseDistances(codeBookIndex);
            int numPoseFeatures = codeBook.poses.numFeaturesFlattened;
            byte* poseCodes = (byte*)codeBook.poses.codes.GetUnsafePtr() + numPoseFeatures * fragmentIndex;
            ...
            if (bTrajectoryMatching)
            {
                float* trajectoryDistances = query.GetTrajectoryDistances(codeBookIndex);
                int numTrajectoryFeatures = codeBook.trajectories.numFeaturesFlattened;
                byte* trajectoryCodes = (byte*)codeBook.trajectories.codes.GetUnsafePtr() + numTrajectoryFeatures * fragmentIndex;

                trajectoryDeviation = 0.0f;
                ptr = trajectoryDistances;
                for (int k = 0; k < numTrajectoryFeatures; ++k)
                {
                    trajectoryDeviation += ptr[*trajectoryCodes++];
        ...
        {
            return self.Cast<MatchFragmentTask>().Execute();
        }

        internal static MatchFragmentTask Create(ref MotionSynthesizer synthesizer, Identifier<PoseSequence> sequences, Identifier<SamplingTime> samplingTime, Identifier<Trajectory> trajectory, Identifier<TimeIndex> closestMatch)
        {
            MemoryArray<PoseSequence> sequencesArray = synthesizer.GetArray<PoseSequence>(sequences);
            if (!sequencesArray.IsValid)
            {
                throw new Exception("Error in MatchFragmentTask : invalid sequences identifier");
            }

            if (sequencesArray.Length == 0)
            {
                throw new Exception("Error in MatchFragmentTask : no sequence matching the query were found in the binary");
            }
